...
class MultifileDownloadsListener
{
    ...
    private null|ContentModel $objElement = null;

    /**
     * Files that will be downloaded.
     */
    private array $arrFileIds = [];
    ...
    public function __invoke(ContentModel $objElement, string $strBuffer, $element): string
    {
        ...
        if ($this->isDownloadFilesRequest()) {
            ...
            $arrValidFileIds = $this->getValidFileIds();
            ...
            foreach ($arrFilePaths as $filePath) {
                ...
                if (null === $objFile) {
                    ...
                    throw new NotFoundException($strText);
                }
    ...
    private function isSendLanguageDataRequest(): bool
    {
        ...
        if ($request->isXmlHttpRequest() && (int)$request->query->get('ce_id') === (int)$this->objElement->id) {
            if ($request->query->has('content_downloads') && $request->query->has('load_language_data')) {
                return true;
            }
        }
    ...
    private function getValidFileIds(): array
    {
        ...
        if ($this->objElement->useHomeDir && $user instanceof FrontendUser) {

            if ($user->assignDir && $user->homeDir) {
                $this->objElement->multiSRC = [$user->homeDir];
            }
        } else {
    ...
    {
        $tmpDir = Path::join($this->projectDir, self::TEMPORARY_FOLDER);
